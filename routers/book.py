...
@book_router.get("", response_model=Response)
def get_books():
    return books


@book_router.get("/{id}", response_model=Response)
def get_book_by_id(id: str):
    book = book_service.get_book_by_id(id)
    if not book:
        raise HTTPException(status_code=404, detail="book not found.")
    return Response(message="Book retrieved successfully", data=book)



@book_router.post("", response_model=Response)
...
@book_router.put("/{id}", response_model=Response)
...
@book_router.delete("/{id}", response_model=Response)
